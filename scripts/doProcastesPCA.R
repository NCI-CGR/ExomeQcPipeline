...
pc <- function(f){
  ...
  write.table(e$vectors[,1:4],paste0("procustesPCs/",outname),col=F,row=F,qu=F)
}
...
r1<-cbind(fam$V1,fam$V2,m0[,1:2])
## pca coordinates of vikings with id
r2<-cbind(rep(groupName,length(pcs)),sapply(basename(pcs),function(x) gsub("_covar.txt","",x)),res[,1:2])
rownames(r2)<-NULL
colnames(r2)<-colnames(r1)
pcaPoints<-rbind(r1,r2)

bitmap(paste0("procrustesPCA",groupName,"_PC1-PC2.png"),res=300)
write.table(pcaPoints,paste0("procrustesPCA",groupName,"_PC1-PC2.txt"),col=F,row=F,qu=F)
...
plot(m0[,1:2],col=pcaColours,lwd=2,ylim=ylim,ylab="PC2",xlab="PC1",main=paste0("procrustes PCA with ",groupName))
points(res[,1:2],pch=4)
...
## pca coordinates of ref individuals with pop label
r1<-cbind(fam$V1,fam$V2,m0[,3:4])
## pca coordinates of vikings with id
r2<-cbind(rep(groupName,length(pcs)),sapply(basename(pcs),function(x) gsub("_covar.txt","",x)),res[,3:4])
rownames(r2)<-NULL
colnames(r2)<-colnames(r1)
pcaPoints<-rbind(r1,r2)

bitmap(paste0("procrustesPCA",groupName,"_PC3-PC4.png"),res=300)
write.table(pcaPoints,paste0("procrustesPCA",groupName,"_PC3-PC4.txt"),col=F,row=F,qu=F)

## should we also be able to have different labels for the samples being procrustered on?

pcaColours<-sapply(fam$V1, function(x) cols[ cols$pop==x,"col"])

plot(m0[,3:4],col=pcaColours,lwd=2,ylim=ylim,ylab="PC4",xlab="PC3",main=paste0("procrustes PCA with ",groupName))
points(res[,3:4],pch=4)
