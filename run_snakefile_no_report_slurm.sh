...
module load python3/3.10.2 singularity
...
sbcmd="sbatch --time=24:00:00 --mem=96g --partition=bigmemq,cgrq --cpus-per-task={threads} --output=${proDir}/logs_${DATE}/snakejob_%j.out"
...
echo "#!/bin/bash" > ${proDir}/logs_${DATE}/run_snakefile_no_report_slurm.sbatch

echo ". /etc/profile.d/modules.sh; module load python3/3.10.2 singularity" >> ${proDir}/logs_${DATE}/run_snakefile_no_report_slurm.sbatch
